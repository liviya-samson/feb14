...
var validateRequired = function validateRequired2(value, required, getErrorMessage) {
...
var generateValidationFunction = function generateValidationFunction2(validationFuncRef, yupSchema, _ref6) {
  var required = _ref6.required, minimum = _ref6.minimum, maximum = _ref6.maximum, minLength = _ref6.minLength, maxLength = _ref6.maxLength, pattern = _ref6.pattern, getErrorMessage = _ref6.getErrorMessage, validateModified = _ref6.validateModified, fieldApi = _ref6.fieldApi, formController = _ref6.formController, scope = _ref6.scope;
...
var informedFormat = function informedFormat2(val, frmtr, old) {
  if (_typeof(frmtr) === "object" && !Array.isArray(frmtr)) {
    ...
    Object.keys(val).forEach(function(key) {
      if (frmtr[key]) {
        var _informedFormatter = informedFormatter(val[key], frmtr[key], old ? old[val] : void 0, val), value = _informedFormatter.value, offset = _informedFormatter.offset;
...
var informedFormatter = function informedFormatter2(val, frmtr, old, full) {
  ...
  (0, import_react6.useEffect)(function() {
    var listener = function listener2(target) {
      if (target === "_ALL_" || scopedFields.includes(target) || target && scopedFields.some(function(field) {
        ...
        debug3("subscription ".concat(event, " triggered with target ").concat(target, " for"), scopedFields);
        cb(target);
      }
    };
  ...
  var wrappedController = (0, import_react12.useMemo)(function() {
    return _objectSpread2(_objectSpread2({}, formController), {}, {
      ...
      getInitialValue: function getInitialValue(fieldName) {
        if (RegExp("".concat(name, "\\[[0-9]+\\]")).test(fieldName)) {
          var path = fieldName.replace(name, "");
          var v = ObjectMap.get(getInitialValues(), path);
          logger2("Resetting ".concat(path, " to ").concat(v));
          return v;
        }
...
var Debug3 = function Debug4(props) {
  ...
  (0, import_react18.useEffect)(function() {
    var _window;
    if ((_window = window) !== null && _window !== void 0 && _window.Prism)
      Prism.highlightAll();
  }, [displayState]);
  ...
  _createClass(FormController2, [{
    ...
    value: function setValue(name, value, e, key, quiet) {
      ...
      if (value === "") {
        ...
        if (meta.formatter) {
          var res = informedFormat(_val, meta.formatter, this.getMaskedValue(name));
    ...
    value: function asyncValidate(done) {
      ...
      this.fieldsMap.forEach(function(meta) {
        ...
        if (_this6.getError(name) === void 0)
          _this6.validateAsync(name);
      });
    }
  }, {
    key: "getErrorMessage",
...
var useForm = function useForm2(_ref) {
  var onSubmit = _ref.onSubmit, onReset = _ref.onReset, onChange = _ref.onChange, onSubmitFailure = _ref.onSubmitFailure, onValueChange = _ref.onValueChange, onValueSet = _ref.onValueSet, onValid = _ref.onValid, onInvalid = _ref.onInvalid, onValueModified = _ref.onValueModified, userInitialValues = _ref.initialValues, validateFields = _ref.validateFields, autocomplete = _ref.autocomplete, showErrorIfError = _ref.showErrorIfError, showErrorIfTouched = _ref.showErrorIfTouched, showErrorIfDirty = _ref.showErrorIfDirty, validateOn = _ref.validateOn, validateOnMount = _ref.validateOnMount, formApiRef = _ref.formApiRef, dontPreventDefault = _ref.dontPreventDefault, yupSchema = _ref.yupSchema, allowEmptyStrings = _ref.allowEmptyStrings, disabled = _ref.disabled, preventEnter = _ref.preventEnter, validateModified = _ref.validateModified, schema = _ref.schema, ajv = _ref.ajv, ajvErrors = _ref.ajvErrors, components = _ref.components, errorMessage = _ref.errorMessage, fieldMap = _ref.fieldMap, adapter = _ref.adapter, name = _ref.name, keepState = _ref.keepState, keepStateIfRelevant = _ref.keepStateIfRelevant, focusOnInvalid = _ref.focusOnInvalid, scrollOnInvalid = _ref.scrollOnInvalid, resetOnlyOnscreen = _ref.resetOnlyOnscreen, userProps = _objectWithoutProperties(_ref, _excluded4);
...
var useConditional = function useConditional2(_ref) {
  var name = _ref.name, evaluate = _ref.evaluate, _ref$evaluateWhen = _ref.evaluateWhen, evaluateWhen = _ref$evaluateWhen === void 0 ? [] : _ref$evaluateWhen, _ref$dependsOn = _ref.dependsOn, dependsOn = _ref$dependsOn === void 0 ? [] : _ref$dependsOn, _ref$native = _ref["native"], _native = _ref$native === void 0 ? false : _ref$native;
  var formController = useFormController();
  ...
  useFieldSubscription(
    ...
    fields,
    function(target) {
      logger5("re-evaluating conditional for ".concat(name, " because of ").concat(target));
      var res = evaluate({
        formState: formController.getFormState(),
        formApi: formController.getFormApi(),
        scope: scopeRef.current,
