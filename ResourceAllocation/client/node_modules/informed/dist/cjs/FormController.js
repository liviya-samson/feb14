  ...
  _rollupPluginBabelHelpers.createClass(FormController, [{
    ...
    value: function focusFirstError() {
      ...
      if (this.options.current.focusOnInvalid) {
        ...
        Array.from(this.fieldsMap.values()).some(function (fieldMeta) {
          ...
          var err = _this5.getError(meta.name);
    ...
    value: function scrollToFirstError() {
      ...
      if (this.options.current.scrollOnInvalid) {
        ...
        Array.from(this.fieldsMap.values()).some(function (fieldMeta) {
          ...
          // Get error for that field
          var err = _this6.getError(meta.name);

          // Only focus if we can
          if (err && formFieldRef) {
            if (typeof formFieldRef.current.scrollIntoView === 'function') {
              debug('Scrolling to', meta.name);
              formFieldRef.current.scrollIntoView(_this6.options.current.scrollOnInvalid);
            }
            return true;
          }
          return false;
    ...
    value: function resetField(name) {
      ...
      if (value) {
        ...
        ObjectMap.ObjectMap.set(this.state.values, name, initializedValue);
        ...
        ObjectMap.ObjectMap.set(this.state.maskedValues, name, maskedValue);
      } else {
        if (resetValue) {
    ...
    value: function reformat(name) {
      ...
      // Get current value
      var currentValue = ObjectMap.ObjectMap.get(this.state.values, name);
      var formatter = meta.formatter,
        parser = meta.parser,
        initialize = meta.initialize;
      ...
      ObjectMap.ObjectMap.set(this.state.values, name, newValue);
      debug("Reformatting ".concat(name, "'s maskedValue to ").concat(newMaskedValue));
      ObjectMap.ObjectMap.set(this.state.maskedValues, name, newMaskedValue);
      this.emit('field', name);
    ...
    value: function valid() {
      ...
      return !!ObjectMap.ObjectMap.empty(errors);
    }
  }, {
    key: "keyDown",
    ...
    value: function validate() {
      ...
      if (this.options.current.yupSchema) {
        var yupErrors = utils.validateYupSchema(this.options.current.yupSchema, values);
        errors = _rollupPluginBabelHelpers.objectSpread2(_rollupPluginBabelHelpers.objectSpread2({}, errors), yupErrors);
      }

      // Validate AJV schema if needed
      if (this.options.current.schema && this.options.current.ajv) {
        var ajvErrors = utils.validateAjvSchema(this.ajvValidate, values);
        errors = _rollupPluginBabelHelpers.objectSpread2(_rollupPluginBabelHelpers.objectSpread2({}, errors), ajvErrors);
      }
      ...
      if (this.options.current.validateFields) {
        ...
        errors = _rollupPluginBabelHelpers.objectSpread2(_rollupPluginBabelHelpers.objectSpread2({}, errors), fieldErrors);
        errors = ObjectMap.ObjectMap.purge(errors);
      }

      // Itterate through and call validate on every field
      this.fieldsMap.forEach(function (fieldMeta) {
        var meta = fieldMeta.current;
        var value = _this7.getValue(meta.name);
        var error = meta.validate ? meta.validate(value, values) : undefined;
        if (error != null) {
          ObjectMap.ObjectMap.set(errors, meta.name, error);
        }
      });
    ...
    value: function asyncValidate(done) {
      ...
      this.fieldsMap.forEach(function (meta) {
        ...
        // Get error to determine if we even want to validateAsync
        if (_this8.getError(name) === undefined) _this8.validateAsync(name);
      });
    }
  }, {
    key: "getErrorMessage",
    value: function getErrorMessage(key, n) {
      ...
      if (this.options.current.schema) {
        ...
        while (name !== '') {
          ...
          var path = utils.getSchemaPathFromJsonPath(name);
          debug("Looking for message at schema path ".concat(path));
          var _property = ObjectMap.ObjectMap.get(this.options.current.schema, path);
    ...
    value: function touchAllFields() {
      ...
      this.fieldsMap.forEach(function (meta) {
        if (!meta.current.arrayField) {
          ...
          ObjectMap.ObjectMap.set(_this9.state.touched, meta.current.name, true);
        }
      });
    }
  }, {
